...
class signin{
	...
	public function reset_password(){
		try{
			$today =  gmdate("Y-m-d\ H:i:s");
			$this->log->logIt($today." ".$this->module."-"."reset password");

			global $tpl;
			global $config;
			$tpl->assign(array(
					"T_BODY"	=> 'reset_password'.$config['tplEx'],
	...
	public function process_reset_password(){
		try{
			$today =  gmdate("Y-m-d\ H:i:s");
			$this->log->logIt($today." ".$this->module."-"."process_reset_password");

			$email = isset($_POST['email']) ? $_POST['email'] : "";
			
			if(!empty($email)){
				
				$response 	= $this->securityservices->fn_ResetPassword($email);
				
				if(empty((array)$response)){
					$response_array['resultStatus'] = resultConstant::Success;
					$response_array['resultData']['message'] = "Mail has been sent to your email account with reset password link, please check your email and click on link to reset password.";
				}else{
					$response_array['resultStatus'] = resultConstant::Success;
					$response_array['resultData']['message'] = $response->faultstring;
				}
				...
				$response_array['resultStatus'] = resultConstant::Warning;
				$response_array['resultData']['message'] = "Error while reset password.";
			}
